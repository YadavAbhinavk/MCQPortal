...
public class QuestionDaoImpl implements QuestionDao{
	...
	public int updateQues(Question question)
	{
		String updateQuery = "Update questions set question_name = ?,option1 = ?, "
				+ "option2 = ?,option3 = ?,option4 = ?,answer=?,correct_marks=?,wrong_marks=? "
				+ "where question_id=?";
		return this.jdbcTemplate.update(
				updateQuery,question.getQuestionName(),question.getOption1(),question.getOption2(),
				question.getOption3(),question.getOption4(),question.getCorrectAnswer(),
				question.getCorrectMarks(),question.getWrongMarks(),question.getQuestionId());
	}
	@Override
